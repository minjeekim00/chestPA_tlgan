...
def create_dst_path(pathfile_feature_direction):
...
def generate_images(pathfile_feature_directions):
    with tf.Session() as sess:
        with open(path_model, 'rb') as file:
            G, D, Gs = pickle.load(file)

        for pathfile_feature_direction in tqdm(pathfile_feature_directions):
            ...
            for cidx, latent in enumerate(latents_n):
                path_gan_version = create_dst_path(pathfile_feature_direction)
...
def generate_images_cam(pathfile_feature_directions):                   
    for pathfile_feature_direction in tqdm(pathfile_feature_directions):
        path_gan_version = create_dst_path(pathfile_feature_direction)
        path_case=path_gan_version+'/'+str(cases[i])
        ...
        for idx, imgs in enumerate(img_all): # case별
            ...
            if os.path.exists(path_dst[idx]):
                continue
            for cidx, cimgs in enumerate(imgs): # class별
                plot_cam(cimgs,plot_fig=True, show_cam=True, save_fig=True, desired_dir=path_dst[idx])
...
generate_images(pathfile_feature_directions)                
generate_images_cam(pathfile_feature_directions)
